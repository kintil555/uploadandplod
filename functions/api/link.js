...
export async function onRequestPost(context) {
  ...
  if (!sourceUrl) return json({ error: 'Link gambar tidak boleh kosong.' }, 400);

  let parsed;
  try { parsed = new URL(sourceUrl); }
  catch { return json({ error: 'Link tidak valid.' }, 400); }
  ...
  if (buffer.byteLength > MAX_SIZE) return json({ error: 'Ukuran gambar maksimal 10MB.' }, 400);
  if (buffer.byteLength === 0)      return json({ error: 'Gambar dari link tersebut kosong.' }, 400);
